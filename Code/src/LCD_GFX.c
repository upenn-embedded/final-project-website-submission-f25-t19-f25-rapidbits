...
void LCD_drawString(uint8_t x, uint8_t y, char* str, uint16_t fg, uint16_t bg)
{
	...
	if (cursorY + 8 >= LCD_HEIGHT) break;  // ???????

	LCD_drawChar(cursorX, cursorY, *str, fg, bg);
	cursorX += 6;   // ???????5?? + 1???
	str++;
    }
	// Fill this out
...
void lcd_drawRect(uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint16_t color)
{
    ...
    LCD_drawLine(x,  y,  x1, y,  color); // top
    LCD_drawLine(x,  y1, x1, y1, color); // bottom
    LCD_drawLine(x,  y,  x,  y1, color); // left
    LCD_drawLine(x1, y,  x1, y1, color); // right
}